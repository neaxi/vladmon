...
# https://forum.micropython.org/viewtopic.php?f=16&t=3675&sid=f03474d606dd98d1f9a3cc75fcc6d99d&start=20#p69631
...
class Policy:
    def __init__(self, hemisphere, week, month, weekday, hour, timezone):
        ...
        if timezone not in range(-780, 781):
            raise ValueError("weekday must be -780..780")
    ...
    # commented out to prevent strings being loaded into RAM -> more free RAM
    ...
    #     ]
    #     return (
    #         "Daylight Saving Policy: daylight saving {} on the {} {} of {} at {:02}:00 hrs (UTC{})"
    #     ).format(
    #         self.prefix,
    #         self.abbr[self.week],
...
class StandardTimePolicy(Policy):
    def __init__(self, hemisphere, week, month, weekday, hour, timezone):
        self.prefix = "ends"
        super(StandardTimePolicy, self).__init__(
            hemisphere, week, month, weekday, hour, timezone
        )
...
class DaylightSavingPolicy(Policy):
    def __init__(self, hemisphere, week, month, weekday, hour, timezone):
        self.prefix = "starts"
        super(DaylightSavingPolicy, self).__init__(
            hemisphere, week, month, weekday, hour, timezone
        )
